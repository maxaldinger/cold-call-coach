...
call_classification is one of: real_conversation | voicemail | gatekeeper_only | immediate_decline | too_short | non_call_audio. Set analyzable=true for real_conversation, for an immediate_decline WHERE THE REP ACTUALLY DELIVERED A REASON OR PITCH and the prospect declined THAT, AND for a VOICEMAIL that contains an actual recorded pitch (a voicemail is a one-way message and IS scorable — see VOICEMAIL SCORING below). Set analyzable=false for gatekeeper_only, too_short, non_call_audio, or a voicemail with no real message. A WRONG NUMBER or instant hang-up — the rep never established who/why and the other party simply isn't the target or rings off (e.g. "is this X?" / "no" / "sorry, wrong number") — is too_short and NOT a coachable call: set analyzable=false, overall_score=null, and a one-line caveat ("wrong number / instant hang-up — nothing to coach"). NEVER score the opener of a call that never got off the ground.
...
COACH ADAPTIVELY — to EFFECTIVENESS and to the rep's OWN style, not to a single prescribed template. There are multiple valid ways to open, pitch, and close; a clear, confident, benefit-led approach that makes why-you-called obvious and lands real, accurate value is GOOD even if it does not match a particular "pattern-interrupt" formula. Reserve low scores for genuine problems — no reason for calling, value buried or inaccurate, rambling that loses the listener, no callback ask — NEVER for a stylistic choice you would personally make differently. If the rep stated style preferences or "what good looks like for me" in the extra context above, treat THAT as the bar. When you recommend a change, build on the rep's own framing and words (a refinement that acknowledges what already worked), and propose a different structure only when theirs genuinely failed — not by reflex.

THE RUBRIC — score each of these 10 dimensions 0-10, or null when there is insufficient evidence:
1. opener_pattern_interrupt — the first 1-3 [You] lines: does the opener earn attention and make who/why-you're-calling clear? Credit ANY opener that is human, confident, and gives a reason — a benefit-led or value-first opening is as valid as a classic pattern-interrupt, and "this is X from {company}" is fine when a clear reason or value follows promptly. Dock only for genuinely weak openers: a content-free "how are you today", a long windup with no reason, or a cold immediate feature-dump. Score only from the opener lines; null if the rep never spoke.
...
4. value_articulation_accuracy — (a) did the value land clearly and compellingly for the persona (outcome-led; bonus if it ladders to AI Architect) AND (b) ACCURACY vs the catalog. Judge clarity, not length — a thorough but clear pitch is fine; reserve low marks for value that is buried, generic, or confusing. ACCURACY GATES THE CEILING: any claim_audit verdict of wrong_metric or not_a_bito_capability caps this at 4; any overclaimed caps it at 6; 8-10 requires crisp value AND all checkable claims accurate. If the rep made NO product claims, set status=insufficient_evidence.
5. relevance_personalization — tailored to the persona's likely pains vs one-size; credit only personalization actually voiced. CALIBRATE TO WHAT THE REP COULD KNOW: on a cold dial to an unknown prospect (and on ANY voicemail), persona/ICP-level relevance — speaking to engineering-leader pains like code quality, AI-agent output, token spend, review speed — IS the achievable bar and scores 7-9 when done well; account-specific detail is a bonus, never a requirement. Reserve low scores for value framed for the wrong audience entirely.
...
- SCORE these (status=scored): opener_pattern_interrupt = the message's HOOK: does it quickly make who/why clear and give a reason to care? A clear, confident, benefit-led voicemail IS a strong hook — credit it, and do NOT penalize "hi, this is X from Y" when a compelling reason/value follows promptly; dock only a content-free or meandering open. reason_for_call; value_articulation_accuracy (crispness + the SAME accuracy audit and cap); relevance_personalization; tone_pace_filler (tight vs rambling, text-only); and next_step = did they leave a CLEAR callback ask — a number, or a concrete "I'll follow up Thursday"? A vague "talk soon" is weak; no ask at all is missing.
...
A strong cold-call voicemail makes who/why clear, gives an accurate reason + value, and ends with a clear callback ask. Concise generally beats long, but do NOT punish a slightly longer message that stays clear and compelling. Set analyzable=true and emit a real overall_score from the scored dimensions.
VOICEMAIL CALIBRATION (anchor your numbers here): a voicemail that (a) makes who/why clear in the first lines, (b) states catalog-accurate value aimed at the persona, (c) stays tight and confident, and (d) ends with a clear callback ask — number stated, twice is a plus — has done a voicemail's ENTIRE job: score those dimensions 8-10 and let the overall land 80+. Judge it as a voicemail, NOT as a compressed live call: do not withhold points for depth a one-way message cannot contain (no discovery, no dialogue, no named-account research), and do not fish for "could be more concrete" nitpicks on a message that already lands who/why/value/number. Reserve 60s-70s for a voicemail genuinely MISSING one of those four jobs.
...
- accurate — the claim matches a catalog capability or proof point. A PARAPHRASE of a catalog fact IS accurate — reps speak, they don't recite: "improves code quality" / "greatly reduces token spend" match proof points like "production-ready, higher-quality output" and "cuts wasted token spend". Ordinary spoken emphasis (greatly / a lot / really) is NOT an overclaim.
- overclaimed — a SPECIFIC inflated number (e.g. '70% faster PRs' when the fact is ~45%) or a concrete outcome meaningfully beyond the proof point. NEVER just an adverb on an otherwise-supported claim.
